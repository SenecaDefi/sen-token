...
contract TokenBridgeScriptL1 is Script {
    ...
    function run() public {
        ...
        StableCoin token = StableCoin(srcToken);
        token.allowEngineContract(msg.sender);
        token.mint(msg.sender, 1 ether);

        //    estimateSendFee(uint16 _dstChainId, bytes32 _toAddress, uint _amount, bool _useZro, bytes calldata _adapterParams) public view virtual override returns (uint nativeFee, uint zroFee) {
        ...
        token.setTrustedRemote(ArbChainId, dstPath);
