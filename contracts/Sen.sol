...
contract SenToken is LayerZeroOFT {
    ...
    function _debitFrom(address from, uint16 , bytes memory , uint amount) internal virtual override returns(uint) {
    ...
    function _creditTo(uint16 , address to, uint amount) internal virtual override returns(uint) {
        // Mint amount to receiver
        _mint(to, amount);
